...
# Threading is due to performance issues and semaphore stuff:
...
def analyze_audio(indata, frames, time_info, status):
    global last_bass_sample, last_mid_sample, last_treble_sample, sample_counter, first_sample_in_window
    ...
    with data_lock:
        ...
        if len(recent_bass) >= 5:
            recent_avg = np.mean(
                list(recent_bass)[:-1]
            )  # Average excluding current value
            samples_since_last_beat = sample_counter - last_bass_sample
        ...
        if beat == 0 and len(recent_mid) >= 5:
            ...
            samples_since_last_beat = sample_counter - last_mid_sample
        ...
        if beat == 0 and len(recent_treble) >= 5:
            ...
            samples_since_last_beat = sample_counter - last_treble_sample
...
def update_plot(frame):
    ...
    if bass_data or mid_data or treble_data:
        ...
        treble_beat_positions = [
            i for i, beat_type in enumerate(beats) if beat_type == 3
        ]
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            " Continuing without Arduino - beat detection will still work but LEDs won't update."
        )
